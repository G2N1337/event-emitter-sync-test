...
class EventHandler extends EventStatistics<EventName> {
  ...
  eventQueue: { eventName: EventName; count: number }[] = [];
  ...
  constructor(emitter: EventEmitter<EventName>, repository: EventRepository) {
    ...
    this.flushInterval = EVENT_SAVE_DELAY_MS
  ...
  addToQueue(eventName: EventName) {
    this.eventQueue.push({ eventName, count: 1 });
  }
  ...
  async flushQueue() {
    ...
    for (const eventName of EVENT_NAMES) {
      if (batchMap[eventName] > 0) {
        try {
          await this.repository.saveMultipleEvents(eventName, batchMap[eventName]);
        } catch (e) {
          this.eventQueue.push({ eventName, count: batchMap[eventName] });
        }
      }
    }
  }
